...
func potential_single(x, y, massive_body):
	...
	var V = - massive_body.mass / massive_body.global_position.distance_to(Vector2(x,y))
	return V
...
func acceleration_single(x, y, massive_body):
	...
	var accel_direction = - (Vector2(x,y) - massive_body.global_position).normalized()
	var a = G * accel_direction * massive_body.mass / massive_body.global_position.distance_squared_to(Vector2(x,y))
	return a
...
func accelerate_reactive_bodies(delta):
	...
	for reactive_body in reactive_bodies:
		var x = reactive_body.global_position.x
		var y = reactive_body.global_position.y
...
func circular_orbit_velocity(reactive_body, massive_body, clockwise = false):
	...
	var linear_speed = sqrt(G * massive_body.mass / reactive_body.global_position.distance_to(massive_body.global_position))
	var velocity_direction = reactive_body.global_position.direction_to(massive_body.global_position).rotated(PI/2)
	if clockwise:
...
func get_orbit(reactive_body, massive_body, timestep = 0.1, max_time_ahead = 5) -> Line2D:
	...
	var x = reactive_body.global_position
...
func get_cm(bodies):
	...
	for gravity_body in bodies:
		cm += gravity_body.mass * gravity_body.global_position
	cm = cm / bodies.size()
	return cm
...
func place_center_of_mass(bodies, location, velocity = Vector2()):
	...
	for gravity_body in bodies:
		gravity_body.global_position += difference
